...
public class Guest implements Serializable, Cloneable {
	...
	public boolean isSameGuest(Guest g) {
		return this.event.getEventId() == g.event.getEventId() && this.guestNumber == g.guestNumber
				&& this.name == g.name;
	}
	...
	/**
	 * The special notes about the relationship between the Guest and the Client
	...
	@JoinTable(name = "whitelist", joinColumns = {
			@JoinColumn(name = "list_owner", referencedColumnName = "guest_id", nullable = false) }, inverseJoinColumns = {
					@JoinColumn(name = "list_member", referencedColumnName = "guest_id", nullable = false) })
	@ManyToMany
	...
	/**
	 * A method for removing from the White List of Guests for the current
	 * Guest.
	 */
	...
	public Boolean moveToTable(EventTable newEventTable) {
		...
		boolean isValidSeatingArrangement = this.event.acquireSA().isValid();
		if (isValidSeatingArrangement == true) {
			return true;
		} else {
