...
public class EventDAO {
	...
	public static List<Event> getEventsForUser(String idNumber, int page, int pageSize) {
		...
		TypedQuery<Event> query = EM.getEM()
				.createQuery("SELECT event FROM event event WHERE event.primaryPlanner = :user", Event.class);
	...
	public static List<Event> getEventsForClient(String idNumber, int page, int pageSize) {
		...
		TypedQuery<Event> query = EM.getEM().createQuery("SELECT event FROM event event WHERE event.client = :client",
				Event.class);
	...
	public static List<Event> getEventsForDate(String datestr, int page, int pageSize) {
		...
		TypedQuery<Event> query = EM.getEM().createQuery(
				"SELECT event FROM event event WHERE event.eventDateTime >= :startDate AND event.eventDateTime < :endDate",
				Event.class);
	...
	public static List<Event> getEventsForClientAndDate(String idNumber, String datestr, int page, int pageSize) {
		...
		LocalDateTime endDate = LocalDateTime.of(date.toLocalDate(), LocalTime.MAX);
		TypedQuery<Event> query = EM.getEM().createQuery(
				"SELECT event FROM event event WHERE event.client = :client AND event.eventDateTime >= :startDate AND event.eventDateTime < :endDate",
				Event.class);
		query.setParameter("client", client);
		query.setParameter("startDate", startDate);
		query.setParameter("endDate", endDate);
	...
	public static List<Guest> getGuestsForEvent(String idNumber, int page, int pageSize) {
		...
		TypedQuery<Guest> query = EM.getEM().createQuery("SELECT guest FROM guest guest WHERE guest.event = :event",
				Guest.class);
